...
def tokenize_multipart_input(
    ...
    if first_sent_limit is not None and len(input_ids) > max_length:
        ...
        logger.warn("Input={} exceeds max_length={} limit: {}".format(len(input_ids), max_length, tokenizer.decode(input_ids)))
    ...
    if prompt:
        ...
        try: 
            ...
            print(f'this is the example ===> {len(input_ids)}, {tokenizer.decode(input_ids)}')
            mask_pos = [input_ids.index(tokenizer.mask_token_id)]
...
class FewShotDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, args, tokenizer, cache_dir=None, mode="train", use_demo=True):
        self.args = args
    ...
    def select_context(self, context_examples):
        ...
        counts = {str(k): 0 for k in self.label_list}
        if len(self.label_list) == 1:
            # Regression
            counts = {'0': 0, '1': 0}
        selection = []
