...
class DynamicTrainingArguments(TrainingArguments):
    ...
    evaluate_during_training: bool = field(
        default=False,
        metadata={"help": "Evaluate model on test data during training"}
    )
...
def main():
    ...
    with open('data/k-shot/toxic/16-13/test.tsv', 'w', encoding='utf-8')as file:
        file.truncate(0)
    with open('data/k-shot/toxic/16-13/test.tsv', 'w', encoding='utf-8')as file:
    ...
    def build_compute_metrics_report_fn(task_name: str, output_dir) -> Callable[[EvalPrediction], Dict]:
        def compute_metrics_fn(p: EvalPrediction):
            ...
            assert (label_ids_avg - label_ids[0]).mean() < 1e-2
            label_ids = label_ids[0]
            print('[CLS]', sentence, 'Is it [MASK] or Not?')
            if preds.item():
                print('The given comment is toxic with probability', round(max(softmax.flatten().tolist()), 4))
            else:
                print('The comment is not toxic with probability', round(max(softmax.flatten().tolist()), 4))
    ...
    if training_args.do_train:
        ...
        if trainer.is_world_process_zero(): # very traas to debug this
    ...
    if training_args.do_predict:
        ...
        for test_dataset in test_datasets:
            trainer.compute_metrics = build_compute_metrics_report_fn(test_dataset.args.task_name, training_args.output_dir)
