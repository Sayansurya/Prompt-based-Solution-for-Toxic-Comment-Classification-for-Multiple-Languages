...
def get_text(template, input_text_tuple, label, tokenizer, mapping):
    ...
    for part in template_list:
        ...
        if part in special_token_mapping:
            ...
            new_tokens += enc(' ' + mapping[str(label)])
        elif part[:5] == 'sent_':
...
def search_template(model, tokenizer, task_name, k, seed, beam, output_dir, data_dir):
    ...
    if task_name in ['SST-2', 'sst-5', 'mr', 'cr', 'subj', 'trec', 'CoLA', 'mpqa', 'toxic']:
...
def main():
    ...
    parser.add_argument('--task_name', type=str, nargs='+', default=["toxic"], help="Task names")
    ...
    # if args.multilingual:
        ...
        # tokenizer = MT5Tokenizer.from_pretrained(args.t5_model)
    # else:
